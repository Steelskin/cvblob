...
namespace cvb {
    ...
    public:
        ...
        //Contour(ContourPolygon &polygon);

        /// \fn void reset(cv::Point startingPoint = cv::Point(0,0), ChainCodes &chainCodes = ChainCodes())
        ...
        cv::Point get_StartingPoint() const; ///< Get the contour starting point
        ChainCodes get_ChainCodes() const;    ///< Get the list of chain codes

        /// \fn ContourPolygon get_ContourPolygon()
        /// \brief returns a polygon structure.
        /// \return A polygon.
        /// \see ContourPolygon
        ContourPolygon get_ContourPolygon() const;

        /// \fn double get_Area(p)
        /// \brief Calculates area of a contour.
        /// \return Area of the contour.
        double get_Area() const;

        /// \fn double get_Perimeter()
        /// \brief Calculates perimeter of a contour.
        /// \return Perimeter of the contour.
        double get_Perimeter() const;

        /// \fn double get_Circularity()
        ...
        /// \fn ContourPolygon get_SimplifiedPolygon(const double delta=1.)
        /// \brief returns a simplified polygon structure reducing the number of vertex according the distance "delta".
        ...
        /// \fn ContourPolygon get_ConvexHull()
        ...
        /// \fn void RenderContour(const cv::Mat &img, const cv::Scalar &color=cv::Scalar(255, 255, 255))
        ...
        /// \fn void WriteCSV(const std::string &filename)
        /// \brief Write a contour to a CSV (Comma-separated values) file.
        /// \param filename File name.
        void WriteCSV(const std::string &filename) const;

        /// \fn void WriteSVG(const std::string &filename, const cv::Scalar &stroke=cv::Scalar(0,0,0), const cv::Scalar &fill=cv::Scalar(255,255,255))
...
#pragma warning(pop)
    ...
    typedef std::shared_ptr<Contour> SharedContour;     ///< Shared contour, to avoid list copy
    typedef std::list<SharedContour> ContoursList; ///< List of contours (chain codes type).

    /// \fn std::ostream &operator<< (std::ostream &output, const cvb::ContourPolygon &p)
